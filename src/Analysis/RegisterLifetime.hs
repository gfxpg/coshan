...
analyzeUtilization cfg = Map.empty --Map.fromList $ utilization <$> blocks
--  where
...
--   writes us [] = us
--   writes us (AsmInstr _ ((Osgpr sgprs) : _) : rest) =
--     writes (Set.union us $ Set.fromList (SGPRWr <$> sgprs)) rest
--   writes us (AsmInstr _ ((Ovgpr vgprs) : _) : rest) =
...
--   readsOp us (Osgpr (r : rest)) = readsOp newUs (Osgpr rest)
--    where
--     newUs | SGPRWr r `Set.member` us = us
--           | otherwise                = Set.insert (SGPRRd r) us
--   readsOp us (Ovgpr (r : rest)) = readsOp newUs (Ovgpr rest)
--    where
