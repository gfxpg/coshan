...
spec = describe "parseInstruction" $ do
  it "extracts registers" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_regs" $
        [i|
  ...
  it "extracts integer literals" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_int_lits" $
        [i|
  ...
  it "extracts float literals" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_float_lits" $
        [i|
  ...
  it "extracts control operands" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_ctrl" $
        [i|
  ...
  it "extracts s_waitcnt operands" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_waitcnt" $
        [i|
          s_waitcnt 0
          s_waitcnt lgkmcnt(7)
  ...
  it "handles instructions with no operands" $ do
    kernel <-
      extractFirstKernel . gfx908Kernel "disasm_zero_op" $
        [i|
          s_endpgm
          v_nop
        |]
